...
class CandidateWays(Base):
    ...
    specialty: Mapped[SpecialityEnum] = mapped_column(
        SAEnum(SpecialityEnum, name="way_specialty"), nullable=False
    )
    ...
    sections = relationship(
        "CandidateWaySections", back_populates="way", cascade="all, delete-orphan"
    )
...
class CandidateWayTags(Base):
    ...
    way_id: Mapped[int] = mapped_column(
        ForeignKey("candidate_ways.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint("way_id", "tag_id", name="uq_candidate_way_tag"),
    )
